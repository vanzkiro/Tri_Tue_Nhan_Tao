def depth_first_search(graph, start_node, goal_node):
    ...
    print("=== TÌM KIẾM THEO CHIỀU SÂU (DFS) ===")
    stt = 1
    ...
    while True:
        ...
        # 2.4. for mỗi trạng thái v kề u do {Đặt v vào đầu danh sách L}
        neighbors = graph.get(u, [])
        # Đảo ngược danh sách kề để khi chèn vào đầu L, đỉnh trái nhất sẽ đứng ở vị trí đầu tiên
        for v in reversed(neighbors):
            L.insert(0, v)     # Đặt v vào đầu danh sách L
...
father_result = depth_first_search(graph_data, start_node='A', goal_node='P')